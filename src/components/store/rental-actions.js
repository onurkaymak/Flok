...
export const fetchRentalService = (reservationInfo, token) => {
  return async (dispatch) => {
    const { rentalServiceId, customerEmail, customerPhoneNum } =
      reservationInfo;
...
export const addRentalService = (reservationInfo, token) => {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/rental", {
        vin: vin,
      ...
      const createdReservation = {
        ...
        pickUpTime: format(
          response.data.customer.rentalJoinEntities[0].reservationStart,
          "Pp",
        ),
        returnTime: format(
          response.data.customer.rentalJoinEntities[0].reservationEnd,
          "Pp",
        ),
